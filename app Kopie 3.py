#!/usr/bin/env python3
"""Zoll-Dateiverwaltung PORT v5.0.1 - SUCF zu S-Anmeldearten hinzugefügt"""
...
def check_initial_setup():
    ...
    if current_config_name and current_config_name in settings:
        ...
        st.session_state['startbuergschaft'] = float(config.get('buergschaft', 13500000))
...
# =============== ORIGINAL CODE AB HIER (NUR MINIMALE ÄNDERUNGEN) ===============
...
def apply_zoelle_rule(results):
    ...
    # Hole Pauschalbetrag aus Settings
    ...
    for row in results:
        ...
        # Fall 2: Gesamtabgaben = 0
            ...
            if row['Zollwert (total)'] > 0:
                ...
                # Kein Zollwert → Pauschale aus Settings
...
def setup_sidebar():
    ...
    with st.sidebar:
        # NEU: Mandanten-Info und Settings-Anzeige
        if 'mandant' in st.session_state:
            ...
            with st.expander("⚙️ Aktuelle Einstellungen", expanded=True):
                ...
                if settings and 'current_config' in settings:
                    ...
                    **Ersatz-Zollsatz:** {config['ersatz_zollsatz']}%
                    ...
                    if st.button("🔧 Einstellungen ändern", use_container_width=True):
        ...
        # Bürgschaft aus Session State anzeigen!
        st.session_state['startbuergschaft'] = st.number_input(
            "Startbürgschaft (€)", 
            min_value=0.0, 
            max_value=100000000.0, 
            value=float(st.session_state.get('startbuergschaft', 13500000.0)),
            step=0.01,
            format="%.2f"
        )
        
        # Bürgschaftserhöhung
        st.session_state['buergschaft_erhöhung_aktiv'] = st.checkbox(
            "Bürgschaftserhöhung am 04.02.2025",
            value=st.session_state.get('buergschaft_erhöhung_aktiv', True),
            help="Erhöht die verfügbare Bürgschaft um 1.500.000 €"
        )
        if st.session_state['buergschaft_erhöhung_aktiv']:
            st.info("✅ Erhöhung um 1.500.000 € wird als Entlastung eingerechnet")
        ...
        # ATB-Filter Info
        st.subheader("🔵 ATB-Filter")
        st.info("""
        Zeilen mit ATB in 'Weitere Registriernummer Folgeverfahren' werden automatisch übersprungen.
        Dies betrifft S-Anmeldearten (SUSP, SUDC, SUCO, SUCF) und andere interne Vorgänge (bis Sept. 2024).
        """)
...
def show_settings_dialog():
    ...
    with st.form("settings_change"):
        ...
        with col3:
            buergschaft = st.number_input(
                "Bürgschafts-Startsumme (€)",
                value=float(current_config.get('buergschaft', 13500000)),
                step=100000.0,
                format="%.2f"
            )
            ersatz_zollsatz = st.number_input(
                "Ersatz-Zollsatz bei 0% (%)",
                value=float(current_config.get('ersatz_zollsatz', 12.0)),
                step=0.1
            )
        ...
        with col4:
            pauschale = st.number_input(
                "Pauschalbetrag (€)",
                value=float(current_config.get('pauschale', 10000)),
                step=1000.0,
                format="%.2f"
            )
    ...
    if count > 0:
        ...
        if uploaded_file is not None:
            with st.spinner("Wird geladen..."):
                ...
                # Validierung
                if not validate_dataframe(df_import, required_cols, file_type):
                    st.stop()
...
def show_processing_button():
    ...
    if (st.session_state.df_import_eza is not None or st.session_state.df_import_zl is not None or 
        ...
        if st.session_state.get('buergschaft_erhöhung_aktiv', False):
            st.info("💰 Bürgschaftserhöhung: +1.500.000 € am 04.02.2025 aktiviert")
